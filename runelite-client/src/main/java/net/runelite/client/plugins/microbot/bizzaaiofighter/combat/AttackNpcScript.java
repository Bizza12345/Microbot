...
public class AttackNpcScript extends Script {
    ...
    @SneakyThrows
    public void run(BizzaAIOFighterConfig config) {
        try {
        ...
        mainScheduledFuture = scheduledExecutorService.scheduleWithFixedDelay(() -> {
            try {
                ...
                if(config.centerLocation().distanceTo(Rs2Player.getWorldLocation()) < config.attackRadius() &&
                        !config.centerLocation().equals(new WorldPoint(0, 0, 0)) &&  BizzaAIOFighterPlugin.getState() != State.BANKING) {
                ...
                if (BizzaAIOFighterPlugin.getCooldown() > 0 || Rs2Combat.inCombat()) {
                    BizzaAIOFighterPlugin.setState(State.COMBAT);
                ...
                if (!attackableNpcs.isEmpty()) {
                    ...
                    BizzaAIOFighterPlugin.setCooldown(config.playStyle().getRandomTickInterval());
