package net.runelite.client.plugins.microbot.bizza.aiofighter.skill;
...
import net.runelite.client.plugins.microbot.bizza.aiofighter.AIOFighterConfig;
import net.runelite.client.plugins.microbot.bizza.aiofighter.enums.AttackStyleOption;
...
public class AttackStyleScript extends Script {
    ...
    private AttackStyleOption attackStyle;
    private AttackStyleOption attackStyleToTrain;
    ...
    private void scheduledTask(AIOFighterConfig config) {
        try {
            ...
            AttackStyleOption fixedStyle = config.fixedAttackStyle();
            if (fixedStyle != AttackStyleOption.DEFAULT) {
                WidgetInfo widget = getWidgetInfoForStyle(fixedStyle);
                if (widget != null && attackStyle != fixedStyle) {
            ...
            if (attackStyle == AttackStyleOption.LONGRANGE || attackStyle == AttackStyleOption.RANGING) {
    ...
    private void updateAttackStyle(int equippedWeaponType, int attackStyleIndex, int castingMode) {
        AttackStyleOption[] attackStyles = getWeaponTypeStyles(equippedWeaponType);
        if (attackStyleIndex < attackStyles.length) {
            attackStyleIndex = (attackStyleIndex == 4) ? attackStyleIndex + castingMode : attackStyleIndex;
            attackStyle = (attackStyles[attackStyleIndex] != null) ? attackStyles[attackStyleIndex] : AttackStyleOption.DEFAULT;
        }
    }

    private AttackStyleOption[] getWeaponTypeStyles(int weaponType) {
        ...
        AttackStyleOption[] styles = new AttackStyleOption[weaponStyleStructs.length];
        int i = 0;
        for (int style : weaponStyleStructs) {
            ...
            AttackStyleOption attackStyle = AttackStyleOption.valueOf(attackStyleName.toUpperCase());

            if (attackStyle == AttackStyleOption.DEFAULT)
            {
            ...
            if (i == 5 && attackStyle == AttackStyleOption.DEFENSIVE)
            {
                attackStyle = AttackStyleOption.DEFENSIVE_CASTING;
            }
    ...
    private boolean isSkillControlled(AttackStyleOption attackStyle) {
    ...
    private WidgetInfo getComponentToDisplay(AIOFighterConfig config) {
        ...
        AttackStyleOption[] attackStyles = getWeaponTypeStyles(equippedWeaponTypeVarbit);
        Random random = new Random();

        // Iterate over attack styles
        for (int i = 0; i < attackStyles.length; i++) {
            AttackStyleOption attackStyle = attackStyles[i];
            if (attackStyle == null) {
                continue;
            }
    ...
    private WidgetInfo getWidgetInfoForStyle(AttackStyleOption style) {
        AttackStyleOption[] attackStyles = getWeaponTypeStyles(equippedWeaponTypeVarbit);
        for (int i = 0; i < attackStyles.length; i++) {
            if (attackStyles[i] == style) {
                switch (i) {
                    case 0:
